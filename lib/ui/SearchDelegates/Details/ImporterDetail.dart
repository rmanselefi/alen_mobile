...
class ImporterDetail extends StatefulWidget {
  final String importerId;

  const ImporterDetail ({ Key key, this.importerId }): super(key: key);

  @override
  _ImporterDetailState createState() => _ImporterDetailState();
}

class _ImporterDetailState extends State<ImporterDetail> {
  ...
  Widget build(BuildContext firstParentContext) {
    ...
    Provider.of<ImporterProvider>(firstParentContext, listen: false);
    ...
    return FutureBuilder<dynamic>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.data == null) {
            ...
            return FutureBuilder<Importers>(
                future: importerProvider.fetchImporters(widget.importerId),
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.none &&
                      snapshot.hasData == null) {
                    return Scaffold(
                                  ...
                                  children: [
                                    ...
                                    Container(
                                      ...
                                      child: Center(
                                        child: Text(
                                            ...
                                            ['Your Health Facility account has no images.'] ??
                                                'Your Health Facility account has no images.'
                                        ),
                                      ),
                                    )
                                    ...
                                    FutureBuilder<List<Category>>(
                                      // future: drugProvider.getCategoryById(pharmacyId),
                                        future: drugProvider.getCategoryImporterById(widget.importerId),
