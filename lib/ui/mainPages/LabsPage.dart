...
class _HospitalPageState extends State<LabsPage> {
  ...
  Future<void> SearchInitializer() async {
    ...
    await Provider.of<SearchProvider>(context, listen: false)
        .fetchAllLabs2(location);

    await Provider.of<SearchProvider>(context, listen: false)
        .getAllSelectedLabServiceTypes2();
  }

  @override
  Widget build(BuildContext context) {
    if(SearchProvider.laboratories2.isEmpty ){
      SearchInitializer();
    }
    ...
    labProvider.getAllSelectedLabServiceTypes();
                                ...
                                children: <Widget>[
                                  GestureDetector(
                                      onTap: () {
                                        ...
                                        hld += SearchProvider.laboratories2;
                                        hld += SearchProvider.labServices2;
                                        showSearch<TrendingSearchable>(
                                            context: context,
                                            delegate: TrendingSearch2(trendings: hld, searchFor: "Search Laboratories"));
